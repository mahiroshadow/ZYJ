...
def arg_process():
    ...
    parser.add_argument('--dataversion', type=str, default='cde_flow',
                        help='the version of input data')  # cde_flow和cde_flow_puzzle
    ...
    parser.add_argument('--learningrate', type=float, default=1e-4, help='the learning rate for training')
...
def tst_model(model, dataloaders, device):
    model.eval()
    ...
    for j, samples in enumerate(dataloaders):
        ...
        end_idx = start_idx + inputs1.shape[0]  # 获取第一个维度大小，即行数
        ...
        predVec[start_idx:end_idx] = predicted  # 把预测的结果放到数组中
...
def main():
    ...
    for subject in subjects:
        ...
        imgDir = os.path.join('data', 'MEGC2019', verFolder, 'MEGC2019_all', '{}_train.txt'.format(subject))
        print(f"imgDir:{imgDir}")

        image_db_train = MEGC2019(imgList=imgDir, transform=data_transforms,transform1=data_transforms1)
        ...
        if optimizer == 'sgd':
            ...
            optimizer_ft = optim.Adam(params_to_update, lr=1e-4,weight_decay=5e-4)
        ...
        # Train and evaluate
        model_ft = train_model(model_ft, dataloader_train, cosine_sim, criterion, optimizer_ft, device,
                               num_epochs=num_epochs)
        torch.save(model_ft, os.path.join('data', 'model_path', 'model_{}.pth').format(subject))

        # Test model
        imgDir = os.path.join('data', 'MEGC2019', verFolder, 'MEGC2019_all', '{}_test.txt'.format(subject))


        image_db_test = MEGC2019(imgList=imgDir, transform=data_transforms,transform1=data_transforms1)

        dataloaders_test = DataLoader(image_db_test, batch_size=batch_size, shuffle=False,
                                                       num_workers=1)

        preds, labels = tst_model(model_ft, dataloaders_test, device)
        preds_np = np.array(preds)
        labels_np = np.array(labels)
        # print(len(preds_np))
        # print(len(labels_np))
        # assert len(preds_np) == len(labels_np)
        allRST.append([preds_np, labels_np])
        acc = torch.sum(preds == labels).double() / len(preds)
        print('Subject {} has the accuracy:{:.4f}'.format(subject, acc))
        print('---------------------------')
        log_f.write('Subject {} has the accuracy:{:.4f}'.format(subject, acc))
